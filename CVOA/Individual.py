...
class Individual:
    ...
    def __init__(self, size_fixed_part, min_size_var_part, max_size_var_part, fixed_part_max_values,
                 var_part_max_value, use_var_part=True, discrete=True):
    ...
    @staticmethod
    def random(size_fixed_part, min_size_var_part, max_size_var_part, fixed_part_max_values, var_part_max_value, use_var_part, discrete):
        ...
        for i in range(size_fixed_part - 1):
            indv.fixed_part[i] = Individual.random_value(discrete, 0, fixed_part_max_values[
        ...
        if use_var_part:
            ...
            indv.fixed_part[-1] = Individual.random_value(discrete, 0, fixed_part_max_values[-1])  # Generate number between 0 and a max (inclusive)
    ...
    def infect(self, travel_distance):
        ...
        infect_varsize = (Individual.random_value(self.discrete, 0, len(
        ...
        if travel_distance < 0:  # Affected elements are ranging from 0 to the length of fixed+var parts (excluding the element that encodes the size of the var-part).
            nmutated = Individual.random_value(self.discrete, 0, total_size - 1)
        else:
